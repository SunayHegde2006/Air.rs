//! Metal Compute Backend — kernels, command encoding, context, scheduler.
//!
//! Simulated Metal infrastructure for Air.rs. All types are pure Rust;
//! no actual `metal` crate dependency. Replace the simulated impls with
//! real `metal` crate bindings when targeting Apple Silicon hardware.
//!
//! Platform-specific types (Metal kernels, MSL sources, command buffers)
//! live here. Platform-agnostic types (`SharedBuffer`, `ComputeBackend`)
//! live in `shared_buffer.rs`. See ADR-0005.
...
pub enum MetalDtype {
    Float32, Float16, BFloat16, Int8, Int32, Uint8, Uint32,
}
...
impl MetalBuffer {
    pub fn new(element_count: usize, dtype: MetalDtype, id: u64) -> MetalResult<Self> {
        ...
        if size_bytes == 0 {
            return Err(MetalError::BufferCreation { size: 0, msg: "zero-size".into() });
        }
        Ok(Self { id, size_bytes, element_count, dtype, data: vec![0u8; size_bytes] })
    }
    ...
    pub fn as_f32(&self) -> MetalResult<&[f32]> {
        if self.dtype != MetalDtype::Float32 {
            return Err(MetalError::UnsupportedDtype(format!("{:?}", self.dtype)));
        }
        Ok(unsafe { std::slice::from_raw_parts(self.data.as_ptr() as *const f32, self.element_count) })
    }
    ...
    pub fn write_f32(&mut self, data: &[f32]) -> MetalResult<()> {
        ...
        if needed > self.size_bytes {
            return Err(MetalError::BufferTooSmall { required: needed, actual: self.size_bytes });
        }
        let bytes = unsafe { std::slice::from_raw_parts(data.as_ptr() as *const u8, needed) };
...
impl fmt::Debug for MetalBuffer {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("MetalBuffer")
            .field("id", &self.id).field("dtype", &self.dtype)
            .field("elements", &self.element_count).field("bytes", &self.size_bytes)
            .finish()
    }
}
